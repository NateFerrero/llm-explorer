...
import { generateNodeContentPrompt } from '@/lib/api-clients';
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      let errorMessage = apiError instanceof Error ? apiError.message : 'Unknown API error occurred';
      
      // Add model-specific context to the error message
      errorMessage = `Failed to generate content for ${node.name || node.id}. ${errorMessage}`;
